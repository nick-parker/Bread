...
public class Reproject {
	 ...
	 * @return A list of Extrusion3D paths generated by projecting path onto 
	 * s.shape.
	 */
	...
	/**
	 * Project a series of 2d extrusions which have already been segmented 
	 * based on the topology of the surface stored in slicer s.
	...
	public ArrayList<Extrusion3D> Proj(ArrayList<Extrusion2D> path){
		...
		for(Extrusion2D e: path){
			...
			if(!travels.isEmpty()){
				...
				travels.clear();
				//Don't continue here, because e isn't part of the travels 
				//we just handled.
			}
	...
	/**
	 * Project a 2D line segment onto s.shape.
	 * @param e 2D extrusion to project.
	 * @param s Slicer to retrieve shape from.
	 * @return An Extrusion3D path on the surface of s.shape with the same 
	 * extrusion type as e.
	 */
	private Extrusion3D projectExtrusion(Extrusion2D e){
		return new Extrusion3D(projectPoint(e.firstPoint()),
							   projectPoint(e.lastPoint()),
							   e.ExtrusionType);
	}
	/**
	...
	private static Extrusion3D setZ(Extrusion2D e, double z){
		return new Extrusion3D(Utils2D.setZ(e.firstPoint(), z),
							   Utils2D.setZ(e.lastPoint(), z),
							   e.ExtrusionType);
	}
	/**
	 * Transforms a series of 2D travel moves length>=1 into a series of 3D 
	 * travel moves which go up to their maximum height then lower straight 
	 * down to the end point, without dropping into any valleys in between.
	 * @param travels
	 * @param s
	 * @return
	 */
	private ArrayList<Extrusion3D> projectTravel(ArrayList<Extrusion2D> travels,
		...
		//Ceiling for travel moves, to avoid unnecessary vertical travel on
		//layers where the surface mesh extends above the model.
		double ModelTop = s.part.boundingBox().getMaxZ()+s.lift+s.layerHeight;
		ArrayList<Extrusion3D> output = new ArrayList<Extrusion3D>();
		Extrusion3D first = projectExtrusion(travels.get(0));
		Point3D last = first.firstPoint();
		double z = last.getZ();
		if(s.lift!=0){
			//If lift is nonzero, we add a travel up by s.lift to the beginning.
			//However, we don't add the actual travel until the end so that the
			//indices of travels and output match up.
			z+=s.lift;
		}
		boolean SlopeDown = false;
		for(Extrusion2D e : travels){
			Extrusion3D proj = projectExtrusion(e);
			if(Utils3D.slope(proj)<0) SlopeDown=true;
			double newZ = proj.lastPoint().getZ()+s.lift;
			
			//TODO Instead of moving the endpoint of travels that go too high,
			//cut them off when they reach the ceiling and add a flat travel
			//afterwards. This prevents collision with the print when using
			//overly simple travels.
			
			//Move up, but not past the top of the part.
			if(newZ>z){
				Point3D next = newZ<=ModelTop ? 
						liftP(proj.lastPoint(),s.lift) :
						new Point3D(proj.lastPoint().getX(),
							    	proj.lastPoint().getY(),
							    	ModelTop);
			...
			for(int i=output.size()-1;i>=0;i--){
				...
				if(newZ<curr.lastPoint().getZ()){
					...
					if(newZ>z&&slope>0){
						...
						Point3D newEnd = new Point3D(lp.getX(),
													 lp.getY(),
													 Math.max(lp.getZ(),z));
						Point3D fp = proj.firstPoint();
						Point3D newStart = new Point3D(fp.getX(),
													   fp.getY(),
													   Math.max(fp.getZ(),z));
						output.set(i,new Extrusion3D(newStart,
													 newEnd,
													 curr.ExtrusionType));
						z=newStart.getZ();
					}
		...
		if(s.lift!=0){
			//If lift is nonzero, add a movement from the actual start of the
			//travel series to +lift above it.
			Extrusion3D lift = new Extrusion3D(first.firstPoint(),
											   liftP(last,s.lift),
											   first.ExtrusionType);
			output.add(0,lift);
			z+=s.lift;
	...
	private Extrusion3D liftE(Extrusion3D e){
		return new Extrusion3D(liftP(e.firstPoint(),to()),
							   liftP(e.lastPoint(),to()),
							   e.ExtrusionType);
	}
	/**
	 * @return The difference between this Reproject instance's offset value 
	 * and shape's current offset.>
	 */
	private double to(){
